...
function stageFor(date) {
  ...
  if (diff < 0) {
    return { stage: 'pre', weekIndex: null, weekday: wd, label: CALENDAR.preLaunch.label, tasks: CALENDAR.preLaunch.tasks, isWeekend };
  }
...
function initScratchpad() {
  ...
  btn.addEventListener('click', async () => {
    ...
    const completedTasks = info.tasks.filter((_, i) => done[i]);
