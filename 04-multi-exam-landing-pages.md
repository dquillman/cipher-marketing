...
| Daily Quiz (5 questions/day) | Unlimited AI Quizzes |
...
- **Will this help me pass Network+ or CySA+ next?** Network+ is fully live — same Bloom's classification and PBQ support, with the Exam Lens. CySA+ is on the roadmap.
...
- [ ] Confirm route exists in `web/src/` and is reachable
- [ ] Confirm GA4 + Meta Pixel + LinkedIn Insight + Google Ads conversion tags fire on `signup_complete` from each LP
- [ ] Confirm pricing block matches `/pricing` byte-for-byte
- [ ] Confirm the "Try a Question" widget question is **original / AI-generated** and not lifted from any third party
- [ ] Confirm hero loads without layout shift on mobile (Tailwind responsive check)
- [ ] Confirm the universal CTA button text is literally "Start Free Trial" — not "Try CipherExam" or any variant
- [ ] Spot-check each FAQ against the verified exam specs in `cipher-exam-context` skill (Full Mock counts, durations, lens names)
